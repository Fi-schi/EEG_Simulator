...
void setupWebServer() {
    ...
    server.on("/processFiles", HTTP_POST,
        ...
        [](AsyncWebServerRequest *request) {
            // Hier wird zusätzlich ein Parameter "channels" erwartet, der für jede Datei den zugehörigen Channel angibt.
            // Die hochgeladenen Dateiinhalte werden kombiniert.
            String allContents = "";
            for (size_t i = 0; i < uploadedFilesContent.size(); i++) {
                allContents += "Datei " + String(i+1) + ":\n" + uploadedFilesContent[i] + "\n\n";
            }
            // Den Channel-Parameter wird hier nicht einzeln pro Datei übergeben, sondern im Dateiverarbeitungsprozess
            // müssten Sie die Zuordnung anhand des mitgesendeten JSON-Arrays (channels) herstellen.
            // Für dieses Beispiel wird zur Vereinfachung angenommen, dass alle Dateien denselben Channel erhalten.
            String channel = "CH_1"; // Standardchannel
            if (request->hasParam("channels", true)) {
                // Hier könnten Sie den JSON-String parsen, um für jede Datei den zugehörigen Channel zu ermitteln.
                // Dies erfordert eine JSON-Bibliothek oder eine eigene einfache Parsing-Lösung.
                // In diesem Beispiel wird der Standardchannel genutzt.
            }
...
String processFileContent(const String& content, const String& channel) {
    ...
    // Zunächst leeren wir das entsprechende Array
    if (channel == "CH_1") {
        CH_1.clear();
    ...
    // Alle Zeichen im Inhalt durchgehen und nur Ziffern extrahieren
    ...
    // Das Array in den Ergebnis-String umwandeln
    if (channel == "CH_1") {
