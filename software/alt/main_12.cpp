...
struct FileData {
  String filename; // z. B. "/Datei.txt"
  String content;  // Inhalt (optional)
};
...
void setupWebServer() {
  ...
  // Upload-Route: Schreibe Datei in SPIFFS und speichere Dateidaten
  server.on("/upload", HTTP_POST,
    ...
    [](AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len, bool final) {
      ...
      if (final) {
        ...
        // Wir speichern hier nur den Dateinamen; den Inhalt lesen wir später aus SPIFFS
  ...
  server.on("/processFiles", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      ...
      for (size_t i = 0; i < uploadedFiles.size(); i++) {
        // Ermittele für jede Datei den zugehörigen Channel anhand des Dateinamens.
      ...
      // Optional: Lösche die verarbeiteten Dateidaten
      uploadedFiles.clear();
...
String getUploadedFilesList() {
  ...
  while (file) {
    ...
    // Entferne den führenden "/" für die Anzeige
    if (name.startsWith("/")) {
...
// Diese Version parst den JSON-String manuell, um den Channel für einen gegebenen Dateinamen zu ermitteln.
// Wir erwarten einen JSON-Array-String wie: 
//   [{"name":"myFile.txt","channel":"CH_2"},{"name":"another.txt","channel":"CH_3"}]
// Dabei wird der führende "/" aus dem Dateinamen entfernt, falls vorhanden.
String parseChannelForFile(const String &channelsJson, const String &filename) {
  String fileChannel = "CH_1"; // Standardwert
  // Entferne führenden "/" aus dem Dateinamen
  String fileNameForMatch = filename;
  if (fileNameForMatch.startsWith("/")) {
    fileNameForMatch = fileNameForMatch.substring(1);
  }
  // Entferne eckige Klammern
  String trimmed = channelsJson;
  trimmed.trim();
  ...
  while(start < trimmed.length()) {
    ...
    String obj = trimmed.substring(start, end);
    // Entferne führendes "{" und ggf. trailing "}"
    obj.trim();
    if(obj.startsWith("{")) obj = obj.substring(1);
    if(obj.endsWith("}")) obj = obj.substring(0, obj.length()-1);
    
    // Suche nach "name":"..."
    int nameIndex = obj.indexOf("\"name\"");
    if(nameIndex != -1) {
      int colon = obj.indexOf(":", nameIndex);
      if(colon != -1) {
        int firstQuote = obj.indexOf("\"", colon);
        int secondQuote = obj.indexOf("\"", firstQuote+1);
        if(firstQuote != -1 && secondQuote != -1) {
          String nameVal = obj.substring(firstQuote+1, secondQuote);
          if(nameVal == fileNameForMatch) {
            // Finde "channel":"..."
            int channelIndex = obj.indexOf("\"channel\"");
            if(channelIndex != -1) {
              int colon2 = obj.indexOf(":", channelIndex);
              if(colon2 != -1) {
                int firstQuote2 = obj.indexOf("\"", colon2);
                int secondQuote2 = obj.indexOf("\"", firstQuote2+1);
                if(firstQuote2 != -1 && secondQuote2 != -1) {
                  return obj.substring(firstQuote2+1, secondQuote2);
                }
              }
            }
          }
