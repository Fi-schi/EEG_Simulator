...
struct FileData {
  ...
  String content;  // Dateiinhalt (optional)
};
...
void setupWebServer() {
  ...
  // Upload-Route: Schreibe Datei in SPIFFS
  ...
  server.on("/processFiles", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      ...
      // [{"name":"Datei.txt","channel":"CH_2"}, {"name":"andere.txt","channel":"CH_3"}]
      String channelsJson = "";
      ...
      for (size_t i = 0; i < uploadedFiles.size(); i++) {
        ...
        if (file) {
          ...
          // Kontrolle: Zähle Ziffern im Originalinhalt
          int digitCount = 0;
...
// Passt den Dateinamen für die Suche im JSON an (entfernt ggf. führenden '/')
String parseChannelForFile(const String &channelsJson, const String &filename) {
  String fileNameForMatch = filename;
  if (fileNameForMatch.startsWith("/")) {
    fileNameForMatch = fileNameForMatch.substring(1);
  }
  // Standardwert ist CH_1
  String fileChannel = "CH_1";
  int pos = channelsJson.indexOf("\"name\":\"" + fileNameForMatch + "\"");
  if (pos != -1) {
