  ...
  server.on("/upload", HTTP_POST,
    ...
    [](AsyncWebServerRequest *request, const String &filename, size_t index, uint8_t *data, size_t len, bool final) {
      ...
      if (final) {
        ...
        FileData fd;
        fd.filename = path;  // z. B. "/myFile.txt"
        // Den Inhalt lesen wir später aus SPIFFS
  ...
  server.on("/processFiles", HTTP_POST,
    [](AsyncWebServerRequest *request) {
      ...
      for (size_t i = 0; i < uploadedFiles.size(); i++) {
        // Ermittele den zugehörigen Channel für diese Datei
...
// Diese Funktion parst den JSON-String manuell, um den Channel für den übergebenen Dateinamen zu ermitteln.
// Erwartetes Format: [{"name":"myFile.txt","channel":"CH_2"},{"name":"another.txt","channel":"CH_3"}]
// Dabei wird der führende "/" aus dem Dateinamen entfernt, damit ein Vergleich möglich ist.
String parseChannelForFile(const String &channelsJson, const String &filename) {
  String fileNameForMatch = filename;
  if (fileNameForMatch.startsWith("/")) {
    fileNameForMatch = fileNameForMatch.substring(1);
  }
  // Standardwert, falls kein Eintrag gefunden wird
  String result = "CH_1";
  
  int pos = 0;
  while (pos < channelsJson.length()) {
    int startObj = channelsJson.indexOf("{", pos);
    if (startObj == -1) break;
    int endObj = channelsJson.indexOf("}", startObj);
    if (endObj == -1) break;
    String obj = channelsJson.substring(startObj, endObj + 1);
    // Suchen nach "name":"..."
    int namePos = obj.indexOf("\"name\"");
    if (namePos != -1) {
      int colonPos = obj.indexOf(":", namePos);
      int firstQuote = obj.indexOf("\"", colonPos);
      ...
      if (firstQuote != -1 && secondQuote != -1) {
        ...
        if (nameVal == fileNameForMatch) {
          int channelPos = obj.indexOf("\"channel\"");
          if (channelPos != -1) {
            int colonPos2 = obj.indexOf(":", channelPos);
            int firstQuote2 = obj.indexOf("\"", colonPos2);
            int secondQuote2 = obj.indexOf("\"", firstQuote2 + 1);
            if (firstQuote2 != -1 && secondQuote2 != -1) {
              result = obj.substring(firstQuote2 + 1, secondQuote2);
              return result;
            }
          }
        }
